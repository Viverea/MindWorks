...
public class SelectCardScriptLevel1 : MonoBehaviour 
{
	private RaycastHit m_rchHit;
	private GameObject m_goFirstCard;
	private GameObject m_goSecondCard;
	
	public int m_nCardsLeft = 12;
	public float m_fTimeLeft = 30.0f;
	public float m_fTotalTime = 30.0f;
	...
	public float m_fRevealTime = 4.0f;
	...
	void Update () 
	{
		if ( !m_bRoundStarted )
		{
			if ( GetComponent<CardMatchTimerScript>().HasEndedTime() )
			{
				...
				GameObject.Find("cardGenerator").GetComponent<GenerateCardScriptLevel1>().Generate();
		...
		if(m_fRevealTime <= 0.0f)
		{
			...
			if(m_bHasPressedTwo == false)
			{
				...
				{
					if (Physics.Raycast (ray, out m_rchHit, 10)) 
					{
						if(m_rchHit.transform.gameObject.tag == "Cards")
						{
							if(m_bHasPressedTwo == false)
							{
								...
								else 
								{
									...
									if( m_goFirstCard && m_goFirstCard != m_goSecondCard)
									{
...
//			}
	...
	{	
		...
		if (m_goFirstCard.name == m_goSecondCard.name)
		{
			...
			m_goTimeCounter.GetComponent<TextMesh>().text = "" + (int) m_fTotalTime;
	...
	IEnumerator Lose ()
	{
		...
		GameObject[] go = GameObject.FindGameObjectsWithTag("Cards");
		for ( int i = 0; i < go.Length; ++i )
		{
			go[i].animation.Play ("reveal");
			Debug.Log(i);
		}
